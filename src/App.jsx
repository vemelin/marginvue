...
function Tooltip({ text, children }) {
  ...
  const toggle = (e) => { e.stopPropagation(); setShow((s) => !s); };
  React.useEffect(() => {
    if (!show) return;
  ...
  return (
    <span ref={ref} onClick={toggle} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}
      style={{ display: "inline-block", cursor: "help", position: "relative" }}>
      {children}
      {show && (
        <span style={{
          position: "fixed",
          background: "#0f1923", color: "#c8d8e8", fontSize: 12,
          padding: "8px 12px", borderRadius: 8, maxWidth: 220,
          whiteSpace: "normal", lineHeight: 1.5,
          border: "1px solid #2a3a4a", boxShadow: "0 4px 20px rgba(0,0,0,0.5)",
          zIndex: 9999, pointerEvents: "none", transform: "translateX(-50%)",
          left: ref.current ? ref.current.getBoundingClientRect().left + ref.current.getBoundingClientRect().width / 2 : "50%",
          top: ref.current ? ref.current.getBoundingClientRect().top - 8 + window.scrollY : 0,
          marginTop: -4, translate: "0 -100%",
        }}>
          {text}
          <span style={{ position: "absolute", bottom: -5, left: "50%", transform: "translateX(-50%)", width: 8, height: 8, background: "#0f1923", border: "1px solid #2a3a4a", borderTop: "none", borderLeft: "none", rotate: "45deg" }} />
        </span>
      )}
    </span>
  );
...
function Card({ children, style = {}, accent = false }) {
  return (
    <div style={{ background: accent ? "linear-gradient(135deg,#0e2235 0%,#0a1a28 100%)" : "#0a1520", border: `1px solid ${accent ? "#1e5080" : "#162030"}`, borderRadius: 12, padding: "20px 22px", ...style }}>
      {children}
    </div>
  );
}
...
function Stat({ label, value, sub, tip, positive, negative, large }) {
  return (
    <div>
      ...
      <div style={{ fontSize: large ? 26 : 20, fontWeight: 700, color: negative ? "#e05a5a" : positive ? "#3dd68c" : "#e2eef8", fontFamily: "'DM Mono', monospace", letterSpacing: "-0.5px" }}>
        {value}
      </div>
...
const btnStyle = (bg, color) => ({ background: bg, color, border: `1px solid ${color}22`, borderRadius: 7, padding: "9px 16px", cursor: "pointer", fontSize: 12, fontWeight: 600, letterSpacing: "0.02em" });

function Dashboard({ acct, setTab }) {
  return (
    <div>
      {acct.hasCall && (
        <div style={{ background: "linear-gradient(90deg,#3a0d0d,#1e0808)", border: "1px solid #7a2020", borderRadius: 10, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 22 }}>⚠️</span>
          <div style={{ flex: 1 }}>
            <div style={{ color: "#ff7070", fontWeight: 700, fontSize: 14 }}>Margin Call Detected</div>
            <div style={{ color: "#cc5050", fontSize: 12, marginTop: 2 }}>{fmt(acct.callAmount)} required by {acct.callDueDate}. Please call <strong style={{ color: "#ff9090" }}>1-800-MARGIN-1</strong> or take action in Margin Profile.</div>
          </div>
          <button onClick={() => setTab("Margin Profile")} style={{ background: "#7a2020", color: "#ffaaaa", border: "none", borderRadius: 7, padding: "8px 14px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>View Details</button>
        </div>
      )}
          ...
          {[["Reg T (Fed)", acct.regT], ["House", acct.house], ["Exchange", acct.exchange]].map(([name, val]) => (
            <div key={name} style={{ marginBottom: 12 }}>
              ...
              <div style={{ height: 5, background: "#162030", borderRadius: 3 }}>
                <div style={{ height: 5, width: `${val}%`, background: val >= 50 ? "#4a9de0" : val >= 30 ? "#f0a830" : "#e05a5a", borderRadius: 3 }} />
              </div>
            </div>
          ))}
        </Card>
      </div>
      <div style={{ marginTop: 10, color: "#2a4050", fontSize: 11, textAlign: "right" }}>Last refreshed: {new Date().toLocaleTimeString()} · Balances reflect available API fields</div>
    </div>
  );
}
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <div>
          ...
          {acct.isDriver && <span style={{ background: "#1e4060", color: "#7ab8e0", fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: 700 }}>DRIVING ACCOUNT</span>}
        </div>
      ...
      {acct.hasCall && (
        <div style={{ background: "linear-gradient(90deg,#3a0d0d,#1e0808)", border: "1px solid #7a2020", borderRadius: 10, padding: "16px 20px", marginBottom: 18 }}>
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
            <span style={{ fontSize: 28 }}>⚠️</span>
            <div style={{ flex: 1 }}>
              <div style={{ color: "#ff7070", fontWeight: 700, fontSize: 15, marginBottom: 6 }}>In Margin Call</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
                <div><div style={{ color: "#883030", fontSize: 11, textTransform: "uppercase" }}>Amount Required</div><div style={{ color: "#ff8080", fontSize: 20, fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{fmt(acct.callAmount)}</div></div>
                <div><div style={{ color: "#883030", fontSize: 11, textTransform: "uppercase" }}>Due By</div><div style={{ color: "#ff8080", fontSize: 16, fontWeight: 700 }}>{acct.callDueDate}</div></div>
                <div><div style={{ color: "#883030", fontSize: 11, textTransform: "uppercase" }}>Consequence</div><div style={{ color: "#cc5050", fontSize: 12 }}>Forced liquidation if unresolved</div></div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 14 }}>
        <Card accent><Stat label="Market Value" value={fmt(acct.marketValue)} tip="Total value of assets eligible for margin" large /></Card>
        <Card accent><Stat label="Margin Debit" value={fmt(acct.marginDebit)} tip="Outstanding margin loan" negative={acct.marginDebit > 0} large /></Card>
        <Card accent><Stat label="Margin Equity" value={fmt(acct.marginEquity)} tip="Market Value minus Margin Debit" positive large /></Card>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 14 }}>
        <Card><Stat label="Cash Buying Power" value={fmt(acct.buyingPowerCash)} tip="Available to trade without margin" /></Card>
        <Card><Stat label="Margin Buying Power" value={fmt(acct.buyingPowerMargin)} tip="Available including margin capacity" /></Card>
        <Card><Stat label="Interest Rate" value={pct(acct.interestRate)} tip="Current annual margin interest rate" /></Card>
        <Card><Stat label="SMA" value={fmt(acct.sma)} tip="Special Memorandum Account balance" /></Card>
      </div>
      <Card style={{ marginBottom: 14 }}>
        <button onClick={() => setExpanded(!expanded)} style={{ background: "none", border: "none", cursor: "pointer", color: "#7ab8e0", fontSize: 13, fontWeight: 600, display: "flex", gap: 8, alignItems: "center", padding: 0 }}>
          <span style={{ transform: expanded ? "rotate(90deg)" : "rotate(0deg)", display: "inline-block", transition: "0.2s" }}>▶</span>
          Requirement Breakdown
        </button>
        {expanded && (
          <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            <div><div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 8 }}>Reg T (Federal)</div><div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{pct(acct.regT)}</div><div style={{ color: "#4a7090", fontSize: 11 }}>Initial requirement</div></div>
            <div><div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 8 }}>House Requirement</div><div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{pct(acct.house)}</div><div style={{ color: "#4a7090", fontSize: 11 }}>Maintenance minimum</div></div>
            <div><div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 8 }}>Maintenance Req.</div><div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmt(acct.maintenanceReqDollar)}</div><div style={{ color: "#4a7090", fontSize: 11 }}>{pct(acct.maintenanceReqPct)} of market value</div></div>
          </div>
        )}
      </Card>
      <div style={{ marginTop: 20 }}>
        ...
        {accounts.map((a) => (
          <div key={a.id} style={{ background: a.isDriver ? "#0e2235" : "#0a1520", border: `1px solid ${a.isDriver ? "#1e5080" : "#162030"}`, borderRadius: 10, padding: "14px 18px", marginBottom: 8, display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", alignItems: "center", gap: 14 }}>
            <div><div style={{ color: "#e2eef8", fontSize: 13, fontWeight: 600 }}>{a.name}</div>{a.isDriver && <span style={{ background: "#1e4060", color: "#7ab8e0", fontSize: 9, padding: "2px 7px", borderRadius: 20, fontWeight: 700 }}>DRIVER</span>}</div>
            <div style={{ textAlign: "right" }}><div style={{ color: "#4a7090", fontSize: 10 }}>Market Value</div><div style={{ color: "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>{fmt(a.marketValue)}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ color: "#4a7090", fontSize: 10 }}>Debit</div><div style={{ color: a.marginDebit > 0 ? "#e05a5a" : "#3dd68c", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>{fmt(a.marginDebit)}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ color: "#4a7090", fontSize: 10 }}>Equity</div><div style={{ color: "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>{fmt(a.marginEquity)}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ color: "#4a7090", fontSize: 10 }}>Rate</div><div style={{ color: "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>{pct(a.interestRate)}</div></div>
          </div>
        ))}
      </div>
    </div>
  ...
  if (confirmed) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        ...
        {newDebit > 0 && <div style={{ background: "#1a1010", border: "1px solid #5a3020", borderRadius: 8, padding: "12px 18px", maxWidth: 400, margin: "0 auto 20px", color: "#cc8060", fontSize: 12 }}>⚠️ Margin call of {fmt(newDebit)} remains.</div>}
  ...
  return (
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 24, alignItems: "center" }}>
        ...
        <div style={{ color: "#4a7090", fontSize: 11, marginLeft: 12 }}>{step === 1 ? "Select type" : step === 2 ? "Amount & landing" : "Confirm"}</div>
      </div>
      ...
      {step === 1 && (
        <Card>
          ...
          {[["dda-invest","Checking → Investment","Move cash from your bank to your investment account"],["invest-dda","Investment → Checking","Withdraw funds from your investment account"],["internal","Investment → Investment","Move between linked investment accounts"]].map(([val, label, desc]) => (
            <div key={val} onClick={() => setType(val)} style={{ padding: "14px 16px", borderRadius: 8, marginBottom: 8, border: `1px solid ${type === val ? "#4a9de0" : "#162030"}`, background: type === val ? "#0e2235" : "#050d14", cursor: "pointer" }}>
          ...
          {acct.marginDebit > 0 && <div style={{ background: "#0e2010", border: "1px solid #2a4020", borderRadius: 8, padding: "10px 14px", marginTop: 12, fontSize: 12, color: "#5a9060" }}>💡 This account has a margin debit of {fmt(acct.marginDebit)}. Transferred funds will automatically pay down this balance.</div>}
      ...
      {step === 2 && (
        <Card>
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Source</div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Destination</div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 4 }}>Amount</div>
          ...
          {acct.marginDebit > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 8 }}>Landing Behavior</div>
          ...
          {amt > 0 && (
            <div style={{ background: "#050d14", border: "1px solid #162030", borderRadius: 8, padding: "14px 16px", marginBottom: 16 }}>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 10 }}>Preview</div>
      ...
      {step === 3 && (
        <Card>
          ...
          <div style={{ color: "#2a4050", fontSize: 10, marginTop: 12, lineHeight: 1.5 }}>By confirming, you agree to applicable margin trading terms and regulations.</div>
      ...
      <Card accent>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 18 }}>
          {[["Starting Cash Equity","equity","$",1000,100000,500],["Reg T Initial Req (%)","loanPct","",25,100,5],["Position Drop (%)","dropPct","",0,80,5]].map(([label, key, prefix, min, max, step]) => (
            <div key={key}>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", marginBottom: 6 }}>{label}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {[["Max Purchase",fmt(maxPurchase),false,false],["Value After Drop",fmt(postDropValue),false,false],["Equity After Drop",fmt(postDropEquity),postDropEquity < 0,postDropEquity > 0],["Equity %",`${postDropEquityPct.toFixed(1)}%`,inCall,!inCall]].map(([label, val, neg, pos]) => (
            <Card key={label}>
              <div style={{ color: "#4a7090", fontSize: 10, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
              <div style={{ color: neg ? "#e05a5a" : pos && !inCall ? "#3dd68c" : "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 700 }}>{val}</div>
            </Card>
          ))}
        </div>
        {inCall ? (
          <div style={{ marginTop: 14, background: "#2a0808", border: "1px solid #7a2020", borderRadius: 8, padding: "12px 16px", color: "#ff7070", fontSize: 13, fontWeight: 600 }}>⚠️ Margin Call! Equity ({postDropEquityPct.toFixed(1)}%) is below the house requirement ({houseReq}%). Deposit {fmt(Math.abs(postDropEquity))} or sell positions to resolve.</div>
        ) : (
          <div style={{ marginTop: 14, background: "#081a10", border: "1px solid #2a5030", borderRadius: 8, padding: "12px 16px", color: "#3dd68c", fontSize: 13, fontWeight: 600 }}>✅ Equity ({postDropEquityPct.toFixed(1)}%) is above the {houseReq}% house requirement. No margin call.</div>
        )}
      </Card>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050d14", color: "#e2eef8", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=DM+Mono:wght@400;500;700&display=swap'); * { box-sizing: border-box; } input[type=range]{cursor:pointer} input[type=number]::-webkit-inner-spin-button{display:none} ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-track{background:#050d14} ::-webkit-scrollbar-thumb{background:#1e3040;border-radius:2px}`}</style>
      <div style={{ background: "#070f18", borderBottom: "1px solid #0e1c28", padding: "0 32px", display: "flex", alignItems: "center", height: 56, gap: 24 }}>
        ...
        {selectedAcct.hasCall && <div style={{ background: "#3a0d0d", border: "1px solid #7a2020", borderRadius: 20, padding: "4px 12px", color: "#ff7070", fontSize: 11, fontWeight: 700, cursor: "pointer" }} onClick={() => setActiveTab("Margin Profile")}>⚠ MARGIN CALL</div>}
      </div>
      <div style={{ background: "#070f18", borderBottom: "1px solid #0e1c28", padding: "0 32px", display: "flex" }}>
        {tabs.map(t => <button key={t} onClick={() => setActiveTab(t)} style={{ background: "none", border: "none", borderBottom: activeTab === t ? "2px solid #4a9de0" : "2px solid transparent", color: activeTab === t ? "#7ab8e0" : "#3a5570", padding: "12px 18px", cursor: "pointer", fontSize: 13, fontWeight: activeTab === t ? 700 : 400 }}>{t}</button>)}
      </div>
