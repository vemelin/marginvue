  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14, marginBottom: 20 }}>
        <Card>
          <Stat label="Cash Available" value={fmt(acct.buyingPowerCash)} tip="Available cash without using margin" />
        </Card>
        <Card>
          <Stat label="Buying Power" value={fmt(acct.buyingPowerMargin)} tip="Includes margin borrowing capacity" sub="w/ margin" />
        </Card>
        <Card>
          <Stat label="Margin Debit" value={fmt(acct.marginDebit)} tip="Outstanding margin loan balance" negative={acct.marginDebit > 0} />
        </Card>
        <Card>
          <Stat label="Net Equity" value={fmt(acct.marginEquity)} tip="Market Value minus Margin Debit" positive={acct.marginEquity > 0} />
        </Card>
      </div>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <div>
          ...
          {acct.isDriver && <span style={{ background: "#1e4060", color: "#7ab8e0", fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: 700, letterSpacing: "0.05em" }}>DRIVING ACCOUNT</span>}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 14 }}>
        <Card accent>
          <Stat label="Market Value" value={fmt(acct.marketValue)} tip="Total value of assets eligible for margin" large />
        </Card>
        <Card accent>
          <Stat label="Margin Debit" value={fmt(acct.marginDebit)} tip="Outstanding margin loan (interest accrues daily)" negative={acct.marginDebit > 0} large />
        </Card>
        <Card accent>
          <Stat label="Margin Equity" value={fmt(acct.marginEquity)} tip="Market Value − Margin Debit" positive large />
        </Card>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 14 }}>
        ...
        <Card><Stat label="SMA" value={fmt(acct.sma)} tip="Special Memorandum Account — available for additional purchases" /></Card>
      </div>
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        {expanded && (
          <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            <div>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>Reg T (Federal)</div>
              <div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{pct(acct.regT)}</div>
              <div style={{ color: "#4a7090", fontSize: 11, marginTop: 4 }}>Initial requirement</div>
            </div>
            <div>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>House Requirement</div>
              <div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{pct(acct.house)}</div>
              <div style={{ color: "#4a7090", fontSize: 11, marginTop: 4 }}>Maintenance minimum</div>
            </div>
            <div>
              <div style={{ color: "#4a7090", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>Maintenance Req.</div>
              <div style={{ color: "#e2eef8", fontSize: 22, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{fmt(acct.maintenanceReqDollar)}</div>
              <div style={{ color: "#4a7090", fontSize: 11, marginTop: 4 }}>{pct(acct.maintenanceReqPct)} of market value</div>
            </div>
          </div>
        )}
      </Card>
    ...
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {[1, 2, 3].map((s) => (
          <div key={s} style={{ display: "flex", alignItems: "center" }}>
            <div style={{
              ...
              background: step >= s ? "#1e5080" : "#0a1520",
              border: `2px solid ${step >= s ? "#4a9de0" : "#162030"}`,
              color: step >= s ? "#7ab8e0" : "#2a4050",
      ...
      {step === 1 && (
        <Card>
          ...
          {[
            ["dda-invest", "Checking → Investment", "Move cash from your bank to your investment account"],
            ["invest-dda", "Investment → Checking", "Withdraw funds from your investment account"],
            ["internal", "Investment → Investment", "Move between linked investment accounts"],
          ].map(([val, label, desc]) => (
            <div
              key={val}
              onClick={() => setType(val)}
              style={{
                ...
                borderRadius: 8,
                marginBottom: 8,
                border: `1px solid ${type === val ? "#4a9de0" : "#162030"}`,
                background: type === val ? "#0e2235" : "#050d14",
                cursor: "pointer",
      ...
      {step === 2 && (
        <Card>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {acct.marginDebit > 0 && (
              <div style={{ color: "#5a9060", fontSize: 11, marginTop: 4 }}>⚠ Has margin debit of {fmt(acct.marginDebit)} — funds will apply to this first</div>
            )}
          </div>
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                placeholder="0.00"
                style={{
                  width: "100%", background: "#050d14", border: "1px solid #162030", borderRadius: 8,
                  padding: "10px 14px 10px 26px", color: "#e2eef8", fontSize: 16,
          ...
          {acct.marginDebit > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              {[
                ["auto", "Auto-apply to margin debit", "Reduces outstanding loan (default)"],
                ["cash", "Deposit to cash balance", "Currently unavailable — custodian required"],
              ].map(([val, label, desc]) => (
                <div
                  key={val}
                  onClick={() => val === "auto" && setLanding(val)}
                  style={{
                    padding: "10px 14px", borderRadius: 8, marginBottom: 6,
                    border: `1px solid ${landing === val ? "#4a9de0" : "#162030"}`,
                    background: landing === val ? "#0e2235" : "#050d14",
                    cursor: val === "auto" ? "pointer" : "not-allowed",
                    opacity: val === "cash" ? 0.5 : 1,
                  }}
                >
          ...
          {amt > 0 && (
            <div style={{ background: "#050d14", border: "1px solid #162030", borderRadius: 8, padding: "14px 16px", marginBottom: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <div style={{ color: "#4a7090", fontSize: 10 }}>Current Margin Debit</div>
                  <div style={{ color: "#e05a5a", fontFamily: "'DM Mono', monospace", fontSize: 15, fontWeight: 700 }}>{fmt(acct.marginDebit)}</div>
                </div>
                <div>
                  <div style={{ color: "#4a7090", fontSize: 10 }}>After Transfer</div>
                  <div style={{ color: newDebit === 0 ? "#3dd68c" : "#f0a830", fontFamily: "'DM Mono', monospace", fontSize: 15, fontWeight: 700 }}>{fmt(newDebit)}</div>
                </div>
                <div>
                  <div style={{ color: "#4a7090", fontSize: 10 }}>Debit Reduced By</div>
                  <div style={{ color: "#3dd68c", fontFamily: "'DM Mono', monospace", fontSize: 15, fontWeight: 700 }}>−{fmt(debitReduced)}</div>
                </div>
                <div>
                  <div style={{ color: "#4a7090", fontSize: 10 }}>Est. Settlement</div>
                  <div style={{ color: "#8aa8c0", fontSize: 12 }}>1–3 business days (ACH)</div>
                </div>
              </div>
            </div>
          )}
      ...
      {step === 3 && (
        <Card>
          ...
          {[
            ["Source", "Checking ****4821"],
            ["Destination", acct.name],
            ["Amount", fmt(amt)],
            ["Landing", "Auto-apply to margin debit"],
            ["New Margin Debit", fmt(newDebit)],
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
        {[
          ["📊", "What is Margin?", "Margin lets you borrow against your portfolio to buy more securities. You pay interest on the borrowed amount."],
          ["⚡", "Buying Power Explained", "Buying power = cash + what you can borrow. With Reg T at 50%, $10k cash gives $20k buying power."],
          ["🚨", "What Triggers a Call?", "When your equity falls below the house maintenance requirement (typically 30%), a margin call is issued."],
          ["💸", "How Transfers Affect Margin", "Depositing cash directly reduces your margin debit dollar-for-dollar, restoring buying power."],
        ].map(([icon, title, body]) => (
          <Card key={title}>
      ...
      <Card accent>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 18 }}>
          {[
            ...
            ["Position Drop (%)", "dropPct", "", 0, 80, 5],
          ].map(([label, key, prefix, min, max, step]) => (
            <div key={key}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                <span style={{ color: "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 700 }}>
                  {calc[key].toLocaleString()}{key !== "equity" ? "%" : ""}
                </span>
              </div>
              <input
                type="range" min={min} max={max} step={step} value={calc[key]}
                onChange={(e) => setCalc((c) => ({ ...c, [key]: parseFloat(e.target.value) }))}
                style={{ width: "100%", accentColor: "#4a9de0" }}
              />
            </div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {[
            ["Max Purchase", fmt(maxPurchase), false, false],
            ["Value After Drop", fmt(postDropValue), false, false],
            ["Equity After Drop", fmt(postDropEquity), postDropEquity < 0, postDropEquity > 0],
            ["Equity %", `${postDropEquityPct.toFixed(1)}%`, inCall, !inCall],
          ].map(([label, val, neg, pos]) => (
            <Card key={label}>
        ...
        {inCall && (
          <div style={{ marginTop: 14, background: "#2a0808", border: "1px solid #7a2020", borderRadius: 8, padding: "12px 16px", color: "#ff7070", fontSize: 13, fontWeight: 600 }}>
            ⚠️ Margin Call! Equity ({postDropEquityPct.toFixed(1)}%) is below the house requirement ({houseReq}%). You would need to deposit {fmt(Math.abs(postDropEquity))} or sell positions to resolve.
          </div>
        )}
        {!inCall && (
          <div style={{ marginTop: 14, background: "#081a10", border: "1px solid #2a5030", borderRadius: 8, padding: "12px 16px", color: "#3dd68c", fontSize: 13, fontWeight: 600 }}>
            ✅ Equity ({postDropEquityPct.toFixed(1)}%) is above the {houseReq}% house requirement. No margin call.
          </div>
        )}
      </Card>
    </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <select
          ...
          onChange={(e) => setSelectedAcct(accounts.find(a => a.id === e.target.value))}
          style={{ background: "#0a1520", border: "1px solid #162030", color: "#8aa8c0", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer" }}
        >
        ...
        <div style={{ color: "#e2eef8", fontFamily: "'DM Mono', monospace", fontSize: 16, fontWeight: 700 }}>
          {fmt(accounts.reduce((s, a) => s + a.marketValue, 0))}
        </div>
        {selectedAcct.hasCall && (
          <div style={{ background: "#3a0d0d", border: "1px solid #7a2020", borderRadius: 20, padding: "4px 12px", color: "#ff7070", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
            onClick={() => setActiveTab("Margin Profile")}>
            ⚠ MARGIN CALL
          </div>
        )}
      </div>
        ...
        {tabs.map(t => (
          <button
            ...
            style={{
              ...
              borderBottom: activeTab === t ? "2px solid #4a9de0" : "2px solid transparent",
              color: activeTab === t ? "#7ab8e0" : "#3a5570",
              padding: "12px 18px",
              cursor: "pointer",
              fontSize: 13,
